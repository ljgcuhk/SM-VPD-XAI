...
df.hourly.forest.shap <- read.csv(paste0(InputFolder, "causalSHAP_result/all_causalSHAP_Forest_dry_2.csv")) %>% 
...
df.hourly.dryland.shap <- read.csv(paste0(InputFolder, "causalSHAP_result/all_causalSHAP_Dryland_dry_1.csv")) %>% 
  rename(Tair_shap = Tair, SW_shap = SW, VPD_shap = VPD, VWC_shap = SWC, Site_ID_shap = Site_ID) %>% 
...
df.hourly.grassland.shap <- read.csv(paste0(InputFolder, "causalSHAP_result/all_causalSHAP_Grassland_dry_2.csv")) %>% 
  rename(Tair_shap = Tair, SW_shap = SW, VPD_shap = VPD, VWC_shap = SWC, Site_ID_shap = Site_ID) %>% 
...
p14 <- ggplot(df.hourly.forest.shap, aes(VWC, VWC_shap)) +
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        legend.position = "none", plot.margin = margin(t = 0.3, r = 0.5, b = 0.3, l = 0.5, unit = "cm"),
        panel.border = element_rect(color = "black", fill = NA, linewidth = 1)) +
  annotate("rect", xmin = 1, xmax = 9, ymin = -6, ymax = 4, 
           color = "black", linetype = "dashed", fill = NA, linewidth = 0.7)

# 1 - Dependence plots for flux shrublands and savannahs ----------------------------------------------
...
# 4 - Dependence plots for global drylands----------------------------------------------
p41 <- ggplot(df.year.dry, aes(SW_ori, SW)) +
...
p42 <- ggplot(df.year.dry, aes(Tair_ori, Tair)) +
  geom_point(aes(color = Land_Cover), alpha = 0.35, size = 0.2) + 
  scale_color_manual(values = my_colors) +
...
p43 <- ggplot(df.year.dry, aes(VPD_ori, VPD)) +
  geom_point(aes(color = Land_Cover), alpha = 0.35, size = 0.2) + 
  labs(x = expression("VPD (hPa)"),
       y = expression('Causal Shapley values')) +
  scale_color_manual(values = my_colors) +
...
p44 <- ggplot(df.year.dry, aes(SWC_ori, SWC)) +
  geom_point(aes(color = Land_Cover), alpha = 0.35, size = 0.2) +
  scale_color_manual(values = my_colors) +
