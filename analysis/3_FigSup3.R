...
for (i in Models){
  for (ii in Groups){
    for (iii in DryWet){
      ...
      df_FI <- data.frame()
      for (k in 1:3) {
      df_FI_ <- read.csv(paste0(InputFolder, "Feature_Importance/FI_",i,"_",ii,"_",iii,"_",k,"_SM1.csv")) %>%
        rename(TA = Tair, VWC = SWC) 
...
for (i in Models){
  for (ii in Groups){
    for (iii in DryWet){
      ...
      df_FI <- data.frame()
      for (k in 1:3) {
        df_FI_ <- read.csv(paste0(InputFolder, "Feature_Importance/FI_",i,"_",iii,"_05_",k,".csv")) %>%
          rename(TA = Tair, VWC = SWC) 
        df_FI <- rbind(df_FI, df_FI_)
      }
      df_FI_sum <- data.frame(TA = mean(df_FI$TA),
...
for (i in Models){
  for (ii in Groups){
    for (iii in DryWet){
      df_name <- paste0("df_", ii, "_", iii, "_", i, "_08")
      df_FI <- data.frame()
      for (k in 1:3) {
        df_FI_ <- read.csv(paste0(InputFolder, "Feature_Importance/FI_",i,"_",iii,"_08_",k,".csv")) %>%
          rename(TA = Tair, VWC = SWC) 
        df_FI <- rbind(df_FI, df_FI_)
      }
      df_FI_sum <- data.frame(TA = mean(df_FI$TA),
...
ggdotchart(df_Forest_dry_causalSHAP, x = "Variable", y = "Mean",                            # Color by groups
                   sorting = "descending",                       # Sort value in descending order
                   add = "segments",                             # Add segments from y = 0 to dots
                   add.params = list(color = "#FB9C2A", size = 3, alpha = 0.1), # Change segment color and size
                   dot.size = 1,                                 # Large dot size
                   label = sprintf("%.2f",round(df_Forest_dry_causalSHAP$Mean,2)),           # Add mpg values as dot labels
...
ggdotchart(df_Forest_wet_causalSHAP, x = "Variable", y = "Mean",                            # Color by groups
                   sorting = "descending",                       # Sort value in descending order
                   add = "segments",                             # Add segments from y = 0 to dots
                   add.params = list(color = "#137DC5", size = 3, alpha = 0.1), # Change segment color and size
                   dot.size = 1,                                 # Large dot size
                   label = sprintf("%.2f",round(df_Forest_wet_causalSHAP$Mean,2)),           # Add mpg values as dot labels
                   font.label = list(color = "black", size = 12,   # Adjust label parameters
                                     vjust = 0.5, hjust = -0.2),
                   legend.title = "",
                   rotate = TRUE,
...
ggdotchart(df_Global_dry_causalSHAP, x = "Variable", y = "Mean",   
                   sorting = "descending",                       
                   add = "segments",                             
...
ggdotchart(df_Global_wet_causalSHAP, x = "Variable", y = "Mean",   
                   sorting = "descending",                       
                   add = "segments",                             
...
ggdotchart(df_Global_dry_causalSHAP_08, x = "Variable", y = "Mean",   
                   sorting = "descending",                       
                   add = "segments",                             
                   add.params = list(color = "#FB9C2A", size = 3, alpha = 0.1), 
                   dot.size = 1,                             
                   label = sprintf("%.2f",round(df_Global_dry_causalSHAP_08$Mean,2)),    
                   font.label = list(color = "black", size = 12, 
                                     vjust = 0.5, hjust = -0.2),
                   legend.title = "",
                   rotate = TRUE,
...
ggdotchart(df_Global_wet_causalSHAP_08, x = "Variable", y = "Mean",   
                   sorting = "descending",                       
                   add = "segments",                             
                   add.params = list(color = "#137DC5", size = 3, alpha = 0.1), 
                   dot.size = 1,                             
                   label = sprintf("%.2f",round(df_Global_wet_causalSHAP_08$Mean,2)),    
                   font.label = list(color = "black", size = 12, 
                                     vjust = 0.5, hjust = -0.2),
                   legend.title = "",
                   rotate = TRUE,
